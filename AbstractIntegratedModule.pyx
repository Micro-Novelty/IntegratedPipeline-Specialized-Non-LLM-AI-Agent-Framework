...
class Transformer:
    ...
    def multi_head_attention(self, x, mask=None, alpha=None):
        ...
        if _OPT_AVAILABLE:
            ...
            Q = optimized_project_heads(x, W_q_mix, B, S, self.num_heads, D, M)
            K = optimized_project_heads(x, W_k_mix, B, S, self.num_heads, D, M)
            V = optimized_project_heads(x, W_v_mix, B, S, self.num_heads, D, M)
        else:
