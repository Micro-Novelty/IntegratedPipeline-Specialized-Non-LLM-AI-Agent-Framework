...
class Transformer:
    ...
    - Forward-pass caching: scaled dot-product attention reuses a cached 1/sqrt(d_k)
      scale factor, and layer norm caches its mean/var so the backward pass doesn't
      recompute them. self.cache is pruned each forward pass (_clear_forward_cache)
      so memory doesn't grow unbounded across many forward() calls (e.g. repeated
      inference requests).
    """
    ...
    def _clear_forward_cache(self, keep_essential=True, max_age_forward_passes=5):
        """
        Prunes stale entries from self.cache so it doesn't grow without bound
        across repeated forward() calls (e.g. many inference requests in a row).

        Cache keys are tagged with the forward-pass number they came from
        (e.g. 'x_attn_input_f12'). Any key tagged with a forward pass older
        than (current_forward_count - max_age_forward_passes) is deleted.

        keep_essential: if True, 'latest_<key>' pointers for essential keys
        (input_ids, mask, alpha, attn_weights, probs, logits) are preserved
        as long as a recent version still exists, since these are commonly
        read right after a forward pass (e.g. for logging or inspection).
        """
    ...
    def _get_attn_scale(self, d_k):
        """
        Returns the cached 1/sqrt(d_k) scaling factor used in scaled dot-product
        attention. d_k never changes after __init__, so this avoids recomputing
        the same sqrt on every attention() call.
        """
    ...
    def attention(self, Q, K, V, mask=None):
        ...
        Optimization note: uses np.einsum instead of explicit transpose + matmul
        (avoids an intermediate K.T allocation), the attention scale is cached via
        _get_attn_scale, and clip/mask/softmax are all done in-place on `scores`
        to avoid allocating new arrays at each step.
        """
        d_k = Q.shape[-1]
    ...
    def multi_head_attention(self, x, mask=None, alpha=None,
        ...
        W_q_mix/W_k_mix/W_v_mix: optional pre-blended projection matrices. forward()
        computes these once and passes them in directly so they aren't recomputed;
        if omitted, they're derived from alpha here as before.

        If the Cython-accelerated module is available (_OPT_AVAILABLE), head
        projection uses optimized_project_heads instead of np.einsum for a faster
        x → (B, heads, T, d_head) reshape/projection.
        """
    ...
    def forward(self, input_ids, AME=None, _update_quality_matrix=None, embedded=False, pad_token_id=0, training=True, attn_dropout=0.1, ffn_dropout=0.1):
        ...
        AME: optionally pass a precomputed AME value (e.g. train() computes it once
        per call instead of once per sample) instead of recomputing it here.

        _update_quality_matrix: controls how often the (expensive) attention
        quality score is recomputed during training — see attention_quality_computing
        throttling below — instead of every single step. If not given, it's derived
        from input consistency, alpha, and ffn_dropout.

        Caching: increments self._forward_count and runs _clear_forward_cache at the
        start of every call so self.cache doesn't grow unbounded across repeated
        forward() calls (e.g. many inference requests). layer_norm_with_cache stores
        the mean/var it computes so dynamic_backward/fixed_attention_backward can
    ...
    def layer_norm_with_cache(self, x, scale, shift, eps=1e-5):
        """
        Same normalization as layer_norm(), but also returns the mean/var it
        computed. forward() stores these in self.cache (e.g. 'ln1_mean',
        'ln1_var') so layer_norm_backward can reuse them directly instead of
        recomputing mean/var from x during the backward pass.
        """
    ...
    def layer_norm_backward(self, d_out, x, scale, shift,
        ...
        mean/var: optionally pass the mean/var already computed and cached by
        layer_norm_with_cache during the forward pass, to avoid recomputing
        them here from x.
        """
        eps = 1e-5
        if mean is None:
    ...
    def train(self, input_ids_list, y_true_list, epochs=100, mode=None,
        ...
        Precomputation (avoids redundant work inside the epoch loop):
        - The learning-rate schedule is computed once upfront into lr_schedule,
          instead of calling learning_rate_warm_up() on every step.
        - AME is computed once for the whole input_ids_list and passed into every
          train_step() call, instead of being recomputed per-sample.
        - Smoothed labels for every sample are precomputed into
          y_true_smoothed_list before the epoch loop starts, instead of calling
          smoothing_labels_utility() on every step.
        """
        losses  = []
        accs    = []
    ...
    def train_step(self, input_ids, epoch, y_true, lr=0.01, AME=None,
        ...
        AME/y_true_smooth: optionally pass values precomputed once by train()
        (AME for the whole batch, smoothed labels for this sample) instead of
        recomputing them on every step.
        """
    ...
    def AME_Encoder(self, x):
        ...
        Uses the Cython-accelerated optimized_ame_encoder when available and x
        is 2-D; otherwise falls back to the numpy implementation below.
        """
        x = np.asarray(x)        
        if _OPT_AVAILABLE and np.asarray(x).ndim == 2:
            return optimized_ame_encoder(np.asarray(x, dtype=np.float64))     
    ...
    def attention_quality_computing(self, attn_weights, AME=None, mask=None):
        ...
        AME: optionally pass a precomputed AME instead of letting this method
        derive it from attn_weights' gradient.

        unsuitable_shape_condition delegates to robust_attention_quality_computing
        (a slower, simpler, non-fused implementation) when attn_weights has an
        odd-sized dimension, since the fused-gradient fast path below assumes
        even dimensions for its vectorized NEON-friendly reductions.
        """
        eps = 1e-5
    ...
    def robust_attention_quality_computing(self, attn_weights, AME=None, mask=None):
        """
        Simpler, unfused fallback for attention_quality_computing — computes the
        same quality score (entropy, max-attention, variance-weighted-by-anisotropy,
        AMR scaling) but calls AME_Encoder/anisotropy_measurement directly instead
        of fusing their np.gradient calls together. Used when attn_weights' shape
        isn't suitable for the fast fused path (see attention_quality_computing).

        Mask handling note: unlike attention_quality_computing, this version does
        not renormalize attn_weights against `mask` before computing statistics —
        mask is accepted for a consistent call signature but currently unused here.
        """
        eps = 1e-5
        eps = 1e-5
        batch, heads, seq_len, _ = attn_weights.shape    
...
# NOTE: OptimizedTransformer is preserved from the documented source for reference,
# but the current pipeline code (ported from the undocumented/new version) wires
# self.model2 up using the plain Transformer class below, not this one — see
# IntegratedPipeline.transformer_utilities(). Flagged for your review.
...
class MLP:
    ...
    def _calibrate_gradient(self, grad, AME, anisotropy):
        """
        Scales the gradient used in the "focused" backward pass by a factor
        derived from AME (Abstract Modelling Energy) and anisotropy, instead
        of applying it unmodified.

        Only kicks in when the gradient's std exceeds 0.1 — i.e. the gradient
        is large/noisy enough that calibration is worth the extra work:
            AEL (abstraction environmental limit) = (1 - AME) * anisotropy
            PRA (possible reflected abstraction)   = (1 - AEL) * std(grad)
            calibration = grad * AEL * PRA, then renormalized to sum to 1

        Falls back to the original, uncalibrated gradient if the result
        contains NaN/Inf (e.g. from a near-zero sum during renormalization),
        so a bad calibration can't silently corrupt training.
        """
        std = np.std(grad)
        eps = 1e-5
...
class IntegratedPipeline:
    ...
    use_async         : If True, sets up an AsyncMessageQueue for non-blocking
                        distributed operations. Note: self._use_async is later
                        re-derived from the USE_ASYNC_QUEUE environment variable
                        (default 'true'), which takes precedence over this argument.
    agent_port        : TCP port for AgentDistributedInference. If None, falls back
                        to the AGENT_PORT environment variable (default 5555) rather
                        than disabling the agent.
    ...
    def _coerce_batch_size(self, batch_size, default=32):
        ...
        Used as the fallback when batch_size isn't already a plain int — e.g.
        it arrived as a 0-d array, a 1-element array, or some other numpy-ish
        wrapper. Unlike a bare int(batch_size), this never raises: if coercion
        fails (TypeError/ValueError) or batch_size is empty, it returns
        `default` instead of crashing the batch prediction call.
        """
        try:
