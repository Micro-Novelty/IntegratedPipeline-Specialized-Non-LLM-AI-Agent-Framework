...
class IntegratedPipeline:
    ...
    def transformer_utilities(self, X_provided= None, X_raw=None, y_true=None, rules=None, 
        ...
        if not unsuitable_training:
            ...
            print(f'[!] Zero ratio in samples: {weak_ratio * 100:.2%}')
            ...
            if self.use_transformer:
                self.model2.train(sequence_inputs, y_true, epochs=self.transformer_training_epochs, mode=mode, lr=lr, embedded=True, batch_size=batch_size, max_samples_for_focused_fit=max_samples_for_focused_fit)
...
class PipelinePredictionManager:
    ...
    def _prepare_train_val_split(self, X, y, min_val_per_class=5,
        ...
        try:
            ...
            print(f'[⚠️] train_test_split failed even without stratify: {e} — '
                f'falling back to all-data-as-training')
            return X, np.empty((0,) + X.shape[1:]), y, np.empty((0,) + y.shape[1:])
        ...
        try:
            ...
            if X is not None and y is not None:
                ...
                if isinstance(y, (str, np.str_)):
                    cleaned_string = y.replace('[', '').replace(']', '').replace('\n', ' ')
                    y = np.fromstring(cleaned_string, sep=' ', dtype=float)
