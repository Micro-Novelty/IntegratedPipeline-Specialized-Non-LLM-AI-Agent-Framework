...
class MLP:
    ...
    def focused_forward(self, x):
        AME = self.AME_Encoder(X)     
        anisotropy = self.anisotropy_measurement(X)
        env_score = (1.0 - AME) * anisotropy 

        for layer in self.feed_layers:
            x = layer.forward(np.asarray(x, dtype=np.float64), env_score)
    ...
    def forward(self, x, AME=None, anisotropy=None):
        ...
        gate_uncertainty = performance_score if performance_score < 1.0 else 0.95
