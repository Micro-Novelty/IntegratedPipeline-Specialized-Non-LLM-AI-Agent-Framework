...
class AgentDistributedInference:
    def __init__(self, pipeline, storage, memory_name, port=5555, 
                 use_async=False, secret_key=None, ssl_cert_file=None, 
                 ssl_key_file=None, shared_auth_token=None, predict_manager=None,
    ...
    def _setup_ssl(self):
        ...
        try:
            ...
            cert_path, key_path, ssl_contexts = self.resolve_and_load_ssl(self.ssl_cert_file, self.ssl_key_file)

            if ssl_contexts:
                self.ssl_cert_file = cert_path
                self.ssl_key_file  = key_path
                self.ssl_context   = ssl_contexts["server"]   # used when accepting connections
                self.client_ssl_context = ssl_contexts["client"]  # used in request_peer_prediction
            else:
...
class IntegratedPipeline:
    def __init__(self, memory_name='agent_memory', use_async=False, 
                  agent_port=None, ssl_cert_file=None, ssl_key_file=None, secret_key=None, 
        ...
        self.distribution = AgentDistributedInference(self, self.storage, memory_name, port=self.port, 
                                                         use_async=use_async, secret_key=self.secret_key, ssl_cert_file=ssl_cert_file, ssl_key_file=ssl_key_file, 
...
class PipelinePredictionManager:
    ...
    def advanced_prediction_method(self, titles, label_map, rules,
        ...
        try:
            ...
            self.pipeline.transformer_utilities(rules, dataset, X, label_map=label_map, y_true=y, batch_size=batch_size)
            input_ids, _ = self.pipeline.input_encoding(dataset)
...
def PermissiveTest():
    ...
    if file:
        pipeline = IntegratedPipeline(file, use_async=True, agent_port=5001, ssl_cert_file=cert_file, ssl_key_file=key_file, bind_host='127.0.0.1', security_level='DEVELOPMENT')
    else:
        print('|| Using original csv_file.pkl file as fallback...')
        pipeline = IntegratedPipeline('csv_file.pkl', use_async=True, agent_port=5001, ssl_cert_file=cert_file, ssl_key_file=key_file, bind_host='127.0.0.1', security_level='DEVELOPMENT')
