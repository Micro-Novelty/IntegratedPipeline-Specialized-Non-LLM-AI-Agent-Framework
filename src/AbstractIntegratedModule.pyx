...
class AdamOptimizer:
    ...
    def step(self, params, grads, lr=None, clip_norm=None):
        """
        params:    dict of {param_name: current_value}  (updated in-place)
        grads:     dict of {param_name: gradient}
        lr:        optional override of self.lr for this step (e.g. for a schedule)
        clip_norm: optional max L2 norm per-parameter gradient clipping
        """
        self.t += 1
        lr = self.lr if lr is None else lr

        # Precompute bias-correction terms once per step
        bc1 = 1 - self.beta1 ** self.t
        bc2 = 1 - self.beta2 ** self.t

        for key in grads:  # only touched params that actually have a gradient
            g = grads[key]
            ...
            # Decoupled weight decay (AdamW), applied directly to the param
        ...
        if training_not_allowed:
            ...
            for epoch in range(epochs):
                ...
                if focused_fit_condition:
                    grad, key_grads = self.focused_backward(grad, self.lr, AME, anisotropy)
                else:
