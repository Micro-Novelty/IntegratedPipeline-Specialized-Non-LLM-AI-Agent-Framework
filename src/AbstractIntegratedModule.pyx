        ...
        try:
            ...
            for c in range(n_classes):
                if isinstance(self.pred_counts[c], (int, float)) and self.pred_counts[c] > 0:
                    ...
                    error_rate    = self.error_counts / (self.pred_counts + 1e-8)
                    reputation    = 1.0 / (1.0 + error_rate)
                    if isinstance(reputation, (list, np.ndarray)):
                        if final_probs.shape != reputation.shape:
                            min_rows = min(final_probs.shape[0], reputation.shape[0])

                            final_probs[:min_rows]
                            reputation[:min_rows]
                            
                    final_probs   *= reputation                       
