...
class HNSW:
    def __init__(self, dim, M=16, ef_construction=100, metric='euclidean', seed=None):
        self.dim = dim
        self.M= M
        self.M0 = 2 * M
    ...
    def _rng_state_to_json(self, state):
        """random.Random.getstate() returns (version, internal_state_tuple,
        gauss_next); JSON can't serialize tuples, so convert the middle
        element to a list."""
    ...
    def to_dict(self) -> dict:
        """Capture full state (data + RNG) needed to exactly reconstruct
        this index, including behavior of any future inserts after load."""
        return {
            "dim": self.dim,
            "M": self.M,
            ...
            "neighbors": [[list(layer_set) for layer_set in node_layers] for node_layers in self.neighbors],
            "max_level_of": self.max_level_of,
...
class PerHeadMemory:
    def __init__(self, n_heads, head_dim, capacity=4000, memory_k=8,
                  metric='euclidean', M=16, evict_fraction=0.1, ef_construction=100,
                  seed=42):
        ...
        self._head_seeds = [(seed + h if seed is not None else None) for h in range(n_heads)]
        self.proper_seed = [self._head_seeds[h] for h in range(n_heads)]
        self.hnsw_arch = HNSW(dim=self.head_dim, M=self.M, ef_construction=self.ef_construction, metric=self.metric, seed=int(self.proper_seed[0]))
        self.indices = [self.hnsw_arch for _ in range(n_heads)]
        self.values = [[] for _ in range(n_heads)]
        self._count = [0] * n_heads


    def write(self, K, V):
        batch, heads, seq, _ = K.shape
        for h in range(heads):
            if self._count[h] >= self.capacity:
                continue
            for b in range(batch):
                for s in range(seq):
                    ...
                    self.indices[h].insert(K[b, h, s])
                    self.values[h].append(np.array(V[b, h, s], copy=True))
                    self._count[h] += 1
            if self._count[h] > self.capacity:
                self._evict_and_rebuild(h)


    def _evict_and_rebuild(self, h):
        keep_from = self.evict_batch
        surviving_vectors = self.indices[h].vectors[keep_from:]
        surviving_values = self.values[h][keep_from:]
        ...
        self.values[h] = surviving_values
    ...
    def retrieve(self, Q):
        ...
        k = self.memory_k
        K_mem = np.zeros((batch, heads, seq, k, d), dtype=np.float32)
        V_mem = np.zeros((batch, heads, seq, k, d), dtype=np.float32)
        mem_mask = np.zeros((batch, heads, seq, k), dtype=np.float32)
        ...
        for h in range(heads):
            ...
            for b in range(batch):
                for s in range(seq):
                    results = self.indices[h].search(Q[b, h, s], k=k, ef=max(k, 50))
                    for j, (node_id, _dist) in enumerate(results):
                        K_mem[b, h, s, j] = self.indices[h].vectors[node_id]
                        V_mem[b, h, s, j] = self.values[h][node_id]
                        mem_mask[b, h, s, j] = 1.0

        return K_mem, V_mem, mem_mask

    def to_dict(self) -> dict:
        """Capture everything needed to exactly reconstruct this memory --
        one HNSW dict per head"""
        return {
            ...
            "seed": self.seed,
            "head_seeds": self._head_seeds,
            "indices": [idx.to_dict() for idx in self.indices],
            "values": [[v.tolist() for v in head_values] for head_values in self.values],
            "count": self._count,
        }
...
class HeaviSide:
    ...
    def heaviside(self, x):
        """Forward spike nonlinearity: 1 where x >= 0, else 0."""
        return (x >= 0.0).astype(np.float64)


    def fast_sigmoid_surrogate(self, x, k=25.0):
        """
        Derivative surrogate based on the "fast sigmoid" sigma(x) = x / (1+k|x|).
        d/dx sigma(x) = 1 / (1 + k|x|)^2

        x is (membrane potential - threshold). k controls steepness: larger k
        -> sharper surrogate, closer to the true (but non-differentiable) step.
        """
        denom = (1.0 + k * np.abs(x))
        return 1.0 / (denom * denom) + self.eps


    def atan_surrogate(self, x, alpha=2.0):
        """
        Derivative surrogate based on the arctangent, used e.g. in SpikingJelly.
        d/dx [ (1/pi) atan(pi/2 * alpha * x) + 0.5 ] = alpha / (2 * (1 + (pi/2 * alpha * x)^2))
        """
        v = (np.pi / 2.0) * alpha * x
        return alpha / (2.0 * (1.0 + v * v) + self.eps)
...
class PoissonEncoder:
    """
    Stochastic rate coding: at every timestep, neuron i fires with
    probability proportional to its (normalized, in [0,1]) input
    intensity, independently sampled each step -> an inhomogeneous
    Poisson-like spike train. This is the most common SNN input encoding
    and is naturally noise-robust since the same input yields a
    different, but statistically similar, spike train on every forward
    pass.
    """

    def __init__(self, n_steps, max_rate=1.0, rng=None):
        self.n_steps = n_steps
        self.max_rate = max_rate
        self.rng = rng or np.random.default_rng()

    def encode(self, x):
        """x : (B, F) array in [0, 1]. Returns (T, B, F) spikes in {0,1}."""
        x = np.clip(x, 0.0, 1.0) * self.max_rate
        T, B, F = self.n_steps, x.shape[0], x.shape[1]
        draws = self.rng.random((T, B, F))
        return (draws < x[None, :, :]).astype(np.float64)


class RateEncoder:
    """
    Deterministic rate coding: neuron i fires regularly, once every
    round(1/intensity_i) steps, giving a reproducible spike train whose
    average rate over T steps equals the input intensity. Useful when
    determinism / low variance is preferred over Poisson's noise.
    """
    ...
    def encode(self, x):
        ...
        T, B, F = self.n_steps, x.shape[0], x.shape[1]
        phase = np.cumsum(np.tile(x[None, :, :], (T, 1, 1)), axis=0)
        prev_count = np.floor(phase - x[None, :, :])
        count = np.floor(phase)
        return (count > prev_count).astype(np.float64)


class LatencyEncoder:
    """
    Temporal / latency coding: stronger inputs fire earlier (a single
    spike per neuron), weaker inputs fire later or not at all. Mirrors
    the "time-to-first-spike" code observed in biological sensory
    systems and is very sparse (at most one spike per neuron).
    """

    def __init__(self, n_steps, tau=1.0, threshold=1e-3):
        self.n_steps = n_steps
        self.tau = tau
        self.threshold = threshold

    def encode(self, x):
        x = np.clip(x, self.threshold, 1.0)
        T, B, F = self.n_steps, x.shape[0], x.shape[1]
        # Higher intensity -> smaller (earlier) spike time.
        spike_time = (self.tau * np.log(1.0 / x)).astype(np.float64)
        spike_time = np.clip(spike_time, 0, T - 1).astype(int)

        spikes = np.zeros((T, B, F))
        b_idx, f_idx = np.meshgrid(np.arange(B), np.arange(F), indexing="ij")
        spikes[spike_time, b_idx, f_idx] = 1.0
        return spikes


class LIFDenseLayer:
    """
    Fully-connected layer of Leaky Integrate-and-Fire spiking neurons.

    Parameters
    ----------
    in_features, out_features : int
    decay : float
        Membrane leak factor beta in (0, 1). beta = exp(-dt/tau_mem).
    threshold : float
        Firing threshold v_th.
    surrogate_fn : callable(x, **kwargs) -> array
        Surrogate derivative of the spike function, evaluated at
        (v[t] - v_th).
    surrogate_kwargs : dict
    weight_scale : float
        Multiplier on the default fan-in-scaled initialization.
    use_bias : bool
    """

    def __init__(self, in_features, out_features, decay=0.9, threshold=1.0
                 , surrogate_kwargs=None,
                 weight_scale=1.0, use_bias=True, rng=None):
        self.in_features = in_features
        self.out_features = out_features
        self.decay = float(decay)
        self.threshold = float(threshold)
        self.heaviside = HeaviSide()
        self.surrogate_fn = self.heaviside.fast_sigmoid_surrogate
        self.surrogate_kwargs = surrogate_kwargs or {}
        self.use_bias = use_bias

        rng = rng or np.random.default_rng()
        # Fan-in scaled uniform init, standard for SNN synapses
        # (keeps membrane potentials in a reasonable dynamic range at t=0).
        limit = weight_scale * np.sqrt(1.0 / in_features)
        self.W = rng.uniform(-limit, limit, size=(in_features, out_features))
        self.b = np.zeros(out_features) if use_bias else None

        self.grad_W = np.zeros_like(self.W)
        self.grad_b = np.zeros_like(self.b) if use_bias else None

        self._cache = None
        self.heaviside = HeaviSide()

    def forward(self, input_spikes, record_rate=False):
        """
        input_spikes : ndarray (T, B, in_features), values in {0,1} (or
                       real-valued currents if used as the first layer).
        returns spikes : ndarray (T, B, out_features)
        """
        T, B, _ = input_spikes.shape
        F_out = self.out_features

        mem = np.zeros((B, F_out))
        prev_spike = np.zeros((B, F_out))

        mem_hist = np.empty((T, B, F_out))
        spike_hist = np.empty((T, B, F_out))

        for t in range(T):
            I_t = input_spikes[t] @ self.W
            if self.use_bias:
                I_t = I_t + self.b
            mem = self.decay * mem + I_t - prev_spike * self.threshold
            spike = self.heaviside.heaviside(mem - self.threshold)
        ...
        if record_rate:
            self.last_firing_rate = spike_hist.mean(axis=(0, 1))
        return spike_hist


    def backward(self, grad_output):
        """
        grad_output : ndarray (T, B, out_features) -- dL/d(spike train)
        returns grad_input : ndarray (T, B, in_features) -- dL/d(input spike train)
        Accumulates self.grad_W, self.grad_b.
        """
        cache = self._cache
        input_spikes = cache["input_spikes"]
        mem_hist = cache["mem_hist"]
        T, B, F_out = grad_output.shape

        self.grad_W = np.zeros_like(self.W)
        if self.use_bias:
            self.grad_b = np.zeros_like(self.b)
        grad_input = np.empty_like(input_spikes)

        grad_mem_future = np.zeros((B, F_out))  # dL/dmem[t+1], starts at 0 for t=T-1

        for t in range(T - 1, -1, -1):
            # Gradient reaching spike[t] from: (a) external/next-layer grad,
            # (b) the reset term in mem[t+1] = ... - spike[t]*v_th
            grad_spike_t = grad_output[t] - grad_mem_future * self.threshold

            sg = self.surrogate_fn(mem_hist[t] - self.threshold, **self.surrogate_kwargs)
            grad_mem_t = grad_spike_t * sg + grad_mem_future * self.decay

            # mem[t] = decay*mem[t-1] + I[t] - spike[t-1]*v_th  =>  dmem[t]/dI[t] = 1
            grad_I_t = grad_mem_t

            grad_input[t] = grad_I_t @ self.W.T
            self.grad_W += input_spikes[t].T @ grad_I_t
            if self.use_bias:
                self.grad_b += grad_I_t.sum(axis=0)

            grad_mem_future = grad_mem_t

        # average weight grads over batch handled by caller/optimizer via batch size scaling
        return grad_input
...
class ReadoutLayer:
    """
    Non-spiking leaky-integrator output layer. Used as the final layer of
    a classifier: it accumulates synaptic current into a membrane trace
    without ever thresholding/spiking, avoiding the extra discretization
    noise a spiking readout would add to the loss.

        v[t] = decay * v[t-1] + s_in[t] @ W + b

    The full membrane trace v (T, B, out_features) is returned; how it is
    reduced to logits (mean / last / max over time) is decided by the
    loss / network layer, since that determines how gradients are routed
    back in through `backward`.
    """

    def __init__(self, in_features, out_features, decay=0.9,
                 weight_scale=1.0, use_bias=True, rng=None,
                 memory_min_neighbors=3.0):
        self.in_features = in_features
        self.out_features = out_features
        self.decay = float(decay)
        self.use_bias = use_bias
    ...
    def forward(self, input_spikes):
        ...
        for t in range(T):
            ...
            mem = self.decay * mem + I_t
            mem_hist[t] = mem
        self._cache = {"input_spikes": input_spikes}

        return mem_hist

    def backward(self, grad_mem_hist):
        """
        grad_mem_hist : (T, B, out_features) dL/dv[t] for every t (caller
        decides how loss gradient is distributed across time, e.g. only
        at t=T-1, or evenly if using a mean readout).
        """
        input_spikes = self._cache["input_spikes"]
        T, B, F_out = grad_mem_hist.shape
        ...
        grad_mem_future = np.zeros((B, F_out))
        for t in range(T - 1, -1, -1):
            grad_mem_t = grad_mem_hist[t] + grad_mem_future * self.decay
            grad_I_t = grad_mem_t
            grad_input[t] = grad_I_t @ self.W.T
            self.grad_W += input_spikes[t].T @ grad_I_t
            if self.use_bias:
...
class SpikeDropout:
    """
    "Locked"/variational dropout for spike trains: a single Bernoulli mask
    per neuron is sampled once per forward pass and applied identically at
    every timestep (standard practice for RNN-like models -- sampling a
    fresh mask per timestep destroys temporal correlations the SNN relies
    on). Surviving units are scaled by 1/(1-p) (inverted dropout).

    No learnable parameters; `backward` just re-applies the same mask.
    """
    ...
    def forward(self, input_spikes):
        if not self.training or self.p == 0.0:
            self._mask = None
            return input_spikes
        _, B, F = input_spikes.shape
        keep = (self.rng.random((1, B, F)) >= self.p).astype(np.float64)
        self._mask = keep / (1.0 - self.p)
        return input_spikes * self._mask
...
class PerSpikeTimingMemory:
    def __init__(self, a_plus=0.01, a_minus=0.012, tau_pre=20.0, tau_post=20.0,
                 w_min=0.0, w_max=1.0, dt=1.0, 
                 K=8, capacity=8000, k=8, metric='euclidean', evict_fraction=0.1):
        self.a_plus = a_plus
    ...
    def write(self, trace_pre, trace_post):
        B, F_in = trace_pre.shape
        for b in range(B): 
            if self._count[b] >= self.mem_capacity:
                continue
            
            if b < len(self.indices):
                self.indices[b].insert(trace_post[b])
                self._count[b] += 1
            if b < len(self.indices):
                self._values[b].append(np.array(trace_pre[b], copy=True))
            
            if self._count[b] >= self.mem_capacity:
                self._evict_and_rebuild(b)

    def _evict_and_rebuild(self, b):
        keep_from = self.evict_batch
        surviving_vectors = self.indices[b].vectors[keep_from:]
        surviving_values = self._values[b][keep_from:]

        fresh_index = HNSW(dim=self.dim, M=self.indices[b].M,
                            ef_construction=self.indices[b].ef_construction,
                            metric=self.metric)
        for vec in surviving_vectors:
            fresh_index.insert(vec)

        self.indices[b] = fresh_index
        self._values[b] = surviving_values
        self._count[b] = len(surviving_values)
            

    def retrieve(self, W, pre_spikes, post_spikes):
        T, B, F_in = pre_spikes.shape
        F_out = post_spikes.shape[2]
        k = self.memory_k
        
        dim = (B * F_out) 
        if not self.indices:
            self.indices = [HNSW(dim=dim, M=24, ef_construction=264, metric=self.metric) for _ in range(dim)]
            self._values = [[] for _ in range(dim)]
            self._count = [0] * dim
            self.dim = dim

        trace_pre = np.zeros((B, k, F_in))
        trace_post = np.zeros((B, k, F_out))
        mem_mask = np.zeros((B, k, F_out))

        for b in range(B):
            if self._count[b] == 0:
                continue
            results = self.indices[b].search(W[b], k=k, ef=max(k, 50))
            for j, (node_id, _dist) in enumerate(results):
                trace_post[b, j] = self.indices[b].vectors[node_id]
                if node_id < len(self._values[b]):
                    trace_pre[b, j] = self._values[b][node_id]
                mem_mask[b, j] = 1.0

        return trace_pre, trace_post, mem_mask



class STDPLearner:
    def __init__(self, in_features, out_features, a_plus=0.01, a_minus=0.012,
                 tau_pre=20.0, tau_post=20.0, w_min=0.0, w_max=1.0, dt=1.0):
        self.a_plus = a_plus
        self.a_minus = a_minus
        self.decay_pre = np.exp(-dt / tau_pre)
        self.decay_post = np.exp(-dt / tau_post)
    ...
    def memory_init(self, ind, pre_spikes, post_spikes, tr_pre, tr_post,
                    trace_post_mem, trace_pre_mem, mem_mask):
        d_B = pre_spikes.shape[-1]
        scale = 1.0 / np.sqrt(d_B)

        pre_logit_growth = 1.0 / (1.0 + np.exp(-tr_pre))
        post_logit_growth = 1.0 / (1.0 + np.exp(-tr_post))
        trans_pre_val = trace_pre_mem * pre_logit_growth[:, np.newaxis, :]
        trans_post_val  = trace_post_mem * post_logit_growth[:, np.newaxis, :]
        
        pre_scores = np.where(mem_mask > 0, trans_pre_val, -1e9)
        pre_scores = pre_scores - pre_scores.max(axis=-1, keepdims=True)
        post_scores = np.where(mem_mask > 0, trans_post_val, -1e9)
        post_scores = post_scores - post_scores.max(axis=-1, keepdims=True)
        
        pre_exp_scores = np.exp(pre_scores) * mem_mask
        pre_weights = (pre_exp_scores / (pre_exp_scores.sum(axis=-1, keepdims=True) + 1e-8)).sum(axis=1)
        post_exp_scores = np.exp(post_scores) * mem_mask
        post_weights = (post_exp_scores / (post_exp_scores.sum(axis=-1, keepdims=True) + 1e-8)).sum(axis=1)

        trace_pre = self.decay_pre * pre_weights + pre_spikes[ind]
        trace_post = self.decay_post * post_weights + post_spikes[ind]

        return trace_pre, trace_post



    def apply(self, W, pre_spikes, post_spikes):
        """
        W : (in_features, out_features) ndarray, updated in place.
        pre_spikes  : (T, B, in_features)
        post_spikes : (T, B, out_features)
        Returns the total weight delta applied (for logging).
        """
        T, B, F_in = pre_spikes.shape
        F_out = post_spikes.shape[2]

        trace_pre = np.zeros((B, F_in))
        trace_post = np.zeros((B, F_out))
        dW = np.zeros_like(W)

        for t in range(T):
            trace_pre = self.decay_pre * trace_pre + pre_spikes[t]
            trace_post = self.decay_post * trace_post + post_spikes[t]

            trace_pre_mem, trace_post_mem, mem_mask = self.memory_spike.retrieve(W, pre_spikes, post_spikes)
            trace_pre_out, trace_post_out = self.memory_init(t, pre_spikes, post_spikes,
                    trace_pre, trace_post,
                    trace_post_mem, trace_pre_mem, mem_mask)

            # potentiation: postsynaptic spike now, weighted by recent presynaptic trace
            dW += self.a_plus * (trace_pre_out.T @ post_spikes[t])
            # depression: presynaptic spike now, weighted by recent postsynaptic trace
            dW -= self.a_minus * (pre_spikes[t].T @ trace_post_out)

        W += dW / B
        np.clip(W, self.w_min, self.w_max, out=W)
        if self.memory_write_enabled:
            self.memory_spike.write(trace_pre, trace_post)
        return dW / B

def softmax_cross_entropy(logits, labels):
    """
    logits : (B, C) ndarray
    labels : (B,) int array of class indices, OR (B, C) one-hot array
    Returns (loss: float, grad_logits: (B, C))
    """
    B = logits.shape[0]
    ...
    if labels.ndim == 1:
        ...
        one_hot_labels[np.arange(B), labels] = 1.0
    else:
        one_hot_labels = labels

    log_probs = shifted - np.log(exp.sum(axis=1, keepdims=True))
    loss = -np.sum(one_hot_labels * log_probs) / B
    grad_logits = (probs - one_hot_labels) / B
    return loss, grad_logits


def mse_loss(pred, target):
    """pred, target: same shape. Returns (loss, grad_pred)."""
    diff = pred - target
...
class SNNNetwork:
    def __init__(self, layers, readout_reduction="mean", input_noise_std=0.0,
                 rng=None):
        """
        layers : list of LIFDenseLayer / SpikeDropout objects, ending in a
                 ReadoutLayer.
        readout_reduction : {"mean", "last", "max"}
            How the readout's (T, B, C) membrane trace is collapsed into
            (B, C) logits for the loss, and correspondingly how the loss
            gradient is redistributed back across time.
        input_noise_std : float
            Std-dev of Gaussian jitter added to the encoded input spike
            train during training only -- a cheap robustness/regularization
            measure (akin to input-noise data augmentation).
        """
        assert isinstance(layers[-1], ReadoutLayer), "last layer must be a ReadoutLayer"
        self.layers = layers
        self.readout_reduction = readout_reduction
        self.input_noise_std = input_noise_std
        self.rng = rng or np.random.default_rng()
        self.training = True
        self.utility = SNNUtility()
    ...
    # ------------------------------------------------------------------
    def _reduce_readout(self, mem_hist):
        """mem_hist: (T, B, C) -> logits (B, C)"""
        if self.readout_reduction == "mean":
            return mem_hist.mean(axis=0)
        elif self.readout_reduction == "last":
            return mem_hist[-1]
        elif self.readout_reduction == "max":
            return mem_hist.max(axis=0)
        else:
            raise ValueError(f"[=] unknown readout_reduction {self.readout_reduction}")

    def _expand_grad(self, grad_logits, mem_hist):
        """grad_logits (B, C) -> grad_mem_hist (T, B, C), matching the
        reduction used in _reduce_readout."""
        T = mem_hist.shape[0]
        if self.readout_reduction == "mean":
            return np.tile(grad_logits[None, :, :] / T, (T, 1, 1))
        elif self.readout_reduction == "last":
            g = np.zeros_like(mem_hist)
            g[-1] = grad_logits
            return g
        elif self.readout_reduction == "max":
            g = np.zeros_like(mem_hist)
            idx = np.argmax(mem_hist, axis=0)  # (B, C)
            B, C = idx.shape
            b_idx, c_idx = np.meshgrid(np.arange(B), np.arange(C), indexing="ij")
            g[idx, b_idx, c_idx] = grad_logits
            return g

    # ------------------------------------------------------------------
    def forward(self, input_spikes):
        """input_spikes: (T, B, F_in). Returns logits (B, C)."""
        x = input_spikes
        if self.training and self.input_noise_std > 0:
            x = x + self.rng.normal(0.0, self.input_noise_std, size=x.shape)
        for layer in self.layers[:-1]:
            x = layer.forward(x)
        mem_hist = self.layers[-1].forward(x)
        self._last_mem_hist = mem_hist
        return self._reduce_readout(mem_hist)

    def backward(self, grad_logits):
        grad = self._expand_grad(grad_logits, self._last_mem_hist)
        for layer in reversed(self.layers):
            grad = layer.backward(grad)
        return grad

    # ------------------------------------------------------------------
    def train_step(self, input_spikes, labels, optimizer, loss_fn=softmax_cross_entropy,
                    max_grad_norm=None, l2_lambda=0.0):
        """One full forward + loss + backward + optimizer.step call."""
        self.train()
        optimizer.zero_grad(self.layers)

        logits = self.forward(input_spikes)
        loss, grad_logits = loss_fn(logits, labels)

        if l2_lambda > 0:
            reg = 0.0
            for layer in self.layers:
                params, _ = layer.parameters()
                if "W" in params:
                    reg += np.sum(params["W"] ** 2)
            loss = loss + 0.5 * l2_lambda * reg
        ...
        if l2_lambda > 0:
            for layer in self.layers:
                ...
                if "W" in params:
                    grads["W"] += l2_lambda * params["W"]

        if max_grad_norm is not None:
            self.utility.clip_grad_norm(self.layers, max_grad_norm)
    ...
    def firing_rates(self):
        """Mean firing rate per layer (for monitoring / homeostasis)."""
        rates = []
        for layer in self.layers[:-1]:
            if hasattr(layer, "_cache") and layer._cache is not None:
                rates.append(layer._cache["spike_hist"].mean())
        return rates


class ThresholdHomeostasis:
    """
    Homeostatic threshold controller for LIFDenseLayer instances.

    After each epoch (or N steps), call `.step(layers)` with the same
    layer list used by the network. Every LIFDenseLayer's scalar
    `threshold` is nudged multiplicatively toward keeping its recorded
    mean firing rate close to `target_rate`:

        if rate too high -> raise threshold (neuron fires less easily)
        if rate too low  -> lower threshold (neuron fires more easily)

    This is a coarse, layer-wide analogue of intrinsic plasticity /
    synaptic scaling seen in biological homeostasis, and is useful as a
    cheap robustness measure against dead or saturated layers, especially
    early in training or under distribution shift.
    """

    def __init__(self, target_rate=0.1, adjust_rate=0.05, min_threshold=0.1,
                 max_threshold=10.0):
    ...
    def step(self, layers):
        for layer in layers:
            if not isinstance(layer, LIFDenseLayer):
                continue
            if layer._cache is None:
                continue
            rate = layer._cache["spike_hist"].mean()
            error = rate - self.target_rate  # >0: firing too much
            layer.threshold *= (1.0 + self.adjust_rate * np.sign(error) * min(abs(error) / max(self.target_rate, 1e-6), 1.0))
            layer.threshold = float(np.clip(layer.threshold, self.min_threshold, self.max_threshold))
    ...
    def step(self, layers):
        for layer in layers:
            ...
            for name, p in params.items():
                ...
                key = (id(layer), name)
                if self.momentum:
                    v = self._velocity.get(key, np.zeros_like(p))
...
class Adam:
    """Standard Adam (Kingma & Ba, 2015), implemented directly in NumPy."""
...
class Transformer:
    def __init__(self, vocab_size, d_model=8, n_heads=2, num_classes=7, learning_rate=0.01, attn_dropout=0.0, ffn_dropout=0.0, weight_decay=1e-4):
        ...
        self.memory = PerHeadMemory(self.n_heads, self.head_dim, capacity=8000,
                                    memory_k=8, metric='euclidean')
...
class KNNAugmentedTransformer(Transformer):
    def __init__(self, *args, memory_capacity=4000, memory_k=8, memory_metric='euclidean', 
                 mem_gate_init_bias=3.0, memory_min_neighbors=None, 
                 **kwargs):
        super().__init__(*args, **kwargs)
        self.head_dim = self.d_model // self.n_heads
        self.memory = self.memory
        self.memory_min_neighbors = memory_min_neighbors if memory_min_neighbors is not None else max(1, memory_k // 2)
        self.mem_gate = np.full(self.n_heads, mem_gate_init_bias)
        self.memory_k = memory_k
        self.memory_write_enabled = True

    @staticmethod
    def memory_attention(Q, K_mem, V_mem, mem_mask):
        d_k = Q.shape[-1]
        scale = 1.0 / np.sqrt(d_k)

        scores = np.einsum('bhsd,bhskd->bhsd', Q, K_mem) * scale
        scores = np.where(mem_mask > 0, scores, -1e9)
        scores = scores - scores.max(axis=-1, keepdims=True)

        exp_scores = np.exp(scores) * mem_mask
        weights = exp_scores / (exp_scores.sum(axis=-1, keepdims=True) + 1e-8)
        out = np.einsum('bhsk,bhskd->bhsd', weights, V_mem)

        return weights, out

    @staticmethod
    def memory_attention_backward(d_mem_out, Q, k_mem, V_mem, mem_weights):
        d_k = Q.shape[-1]
        scale = 1.0 / np.sqrt(d_k)

        d_weights = np.einsum('bhsd,bhskd->bhsd', d_mem_out, k_mem)
        d_scores = mem_weights * (d_weights - np.sum(d_weights * mem_weights, axis=-1, keepdims=True))
        d_scores = d_scores * scale

        d_Q = np.einsum('bhsd,bhskd->bhsd', d_scores, k_mem)
        return d_Q
    ...
    def multi_head_attention(self, x, mask=None, alpha=None,
        ...
        local_out, local_weights = self.attention(Q, K, V, mask)
        self.cache['local_out'] = local_out
        self.cache['attn_weights'] = local_weights

        K_mem, V_mem, mem_mask = self.memory.retrieve(Q)
        mem_weights, mem_out = self.memory_attention(Q, K_mem, V_mem, mem_mask)
        self.cache['K_mem'] = K_mem
        self.cache['V_mem'] = V_mem
        self.cache['mem_mask'] = mem_mask
        self.cache['mem_out'] = mem_out
        self.cache['mem_weights'] = mem_weights

        has_memory = (mem_mask.sum(axis=-1, keepdims=True) >= self.memory_min_neighbors).astype(np.float32)
        gate = self._sigmoid(self.mem_gate).reshape(1, self.n_heads, 1, 1)
        gate_used = has_memory * gate + (1.0 - has_memory) * 1.0
        self.cache['gate'] = gate
        self.cache['has_memory'] = has_memory

        attn_output = gate_used * local_out + (1.0 - gate_used) * mem_out
        self.cache['attn_output'] = attn_output

        attn_output_c = attn_output.transpose(0, 2, 1, 3).reshape(batch_size, seq_len, -1)
        self.cache['attn_concat'] = attn_output_c

        output = np.matmul(attn_output_c, self.W_o)
        self.cache['attn_out'] = output

        return output, local_weights

    def predict(self, input_ids, embedded=False):
        prev = self.memory_write_enabled
        self.memory_write_enabled = False
        try:
            return super().predict(input_ids, embedded=embedded)
        finally:
            self.memory_write_enabled = prev

    def _gate_and_local_mem_grads(self, d_combined):
        gate = self.cache['gate']
        has_memory = self.cache['has_memory']
        local_out = self.cache['local_out']
        mem_out = self.cache['mem_out']

        d_local_out = d_combined * (has_memory * gate + (1.0 - has_memory))
        d_mem_out = d_combined * (has_memory * (1.0 - gate))

        d_gate_pre = np.sum(d_combined * (local_out - mem_out) * has_memory, axis=(0, 2, 3))
        gate_flat = gate.reshape(-1)
        d_gate_logit = d_gate_pre * gate_flat * (1.0 - gate_flat)

        return d_local_out, d_mem_out, d_gate_logit
    ...
    def dynamic_backward(self, d_logits, lr=0.01, max_norm=1.0):
        ...
        if mask is not None:
            token_mask = mask[:, 0, 0, :, np.newaxis]
            lengths = token_mask.sum(axis=1, keepdims=True)
            d_x = (d_pooled[:, np.newaxis, :] / (lengths + 1e-6)) * token_mask
        else:
            d_x = np.repeat(d_pooled[:, np.newaxis, :] / self.cache['seq_len'], self.cache['seq_len'], axis=1)
        ...
        d_ln1 = self.layer_norm_backward(
            d_prev + d_x, self.cache['x_ln1_input'], self.ln1_scale, self.ln1_shift
        )

        d_residual = d_ln1
        d_attn = d_ln1
        dx = d_prev + d_residual  # noqa: F841 (kept for parity with your original)
        ...
        d_head = self.n_heads
        d_dim = self.d_model // self.n_heads
        d_combined = d_attn_concat.reshape(batch, seq_len, d_head, d_dim).transpose(0, 2, 1, 3)

        # ---- NEW: split across gate, get memory-branch Q gradient ----
        d_local_out, d_mem_out, d_gate_logit = self._gate_and_local_mem_grads(d_combined)
        ...
        d_V = np.matmul(weight.transpose(0, 1, 3, 2), d_local_out)
        d_weights = np.matmul(d_local_out, V.transpose(0, 1, 3, 2))
        d_scores = weight * (d_weights - np.sum(d_weights * weight, axis=-1, keepdims=True))
        d_scores /= np.sqrt(Q.shape[-1])
        d_Q_local = np.matmul(d_scores, K)
        d_K = np.matmul(d_scores.transpose(0, 1, 3, 2), Q)

        d_Q_mem = self.memory_attention_backward(
            d_mem_out, Q, self.cache['K_mem'], self.cache['V_mem'], self.cache['mem_weights']
        )
        d_Q = d_Q_local + d_Q_mem
        # ---- end new ----
        ...
        self.W_k = self.apply_update(self.W_k, grads['W_k'], lr)
        self.W_v = self.apply_update(self.W_v, grads['W_v'], lr)
        self.mem_gate = self.apply_update(self.mem_gate, grads['mem_gate'], lr)  # NEW
    ...
    def fixed_attention_backward(self, d_logits, lr=0.01, max_norm=1.0):
        ...
        # ---- NEW: gate still trains in fixed mode (Q/K/V stay frozen, gate doesn't) ----
        batch, seq_len, _ = d_attn.shape
        d_head = self.n_heads
        d_dim = self.d_model // self.n_heads
        d_combined = d_attn.reshape(batch, seq_len, d_head, d_dim).transpose(0, 2, 1, 3)
        _, _, d_gate_logit = self._gate_and_local_mem_grads(d_combined)
        # ---- end new ----
...
class AdamOptimizer:
    ...
    def step(self, params, grads, lr=None, clip_norm=None):
        ...
        for key in grads:
            ...
            # distinguish "params also disagree" (a real backward() bug) from the moment-buffer case.
...
class MLP:
    ...
    def train(self, X, y, X_val=None, y_val=None, epochs=1000, lr=0.01, verbose=True, max_samples_for_focused_fit=500):
        y_val_pred = None
        y_val_true = None
        X = self._sanitize_string_chars(X)
        ...
        if training_not_allowed:
            ...
            for epoch in range(epochs):
                if X_val is not None and y_val is not None:
                    y_val_pred = self.forward(X_val, y=y_val, AME=AME, anisotropy=anisotropy, condition='training')
                    y_val_pred, y_val_true = self.adapt_predict_shape(y_val_pred, y_val)
                ...
                try:
                    ...
                    print(f'[>] AdamOptimizer error occured: {e}')
                    continue
                
                if verbose and epoch % 100 == 0:
                    acc = np.mean(np.argmax(y_pred, axis=1) == np.argmax(y_true, axis=1))
                    if y_val_pred is not None and y_val_true is not None:
                        val_acc = np.mean(np.argmax(y_val_pred, axis=1) == np.argmax(y_val_true, axis=1))
                        print(f"[=] Epoch {epoch} | Loss: {loss:.4f} | Acc: {acc:.2f} | Validation Acc: {val_acc:.2f}")
                    else:
                        print(f"[=] Epoch {epoch} | Loss: {loss:.4f} | Acc: {acc:.2f}")
...
class ExplainabilityModule:
    ...
    def _get_final_output(self, mlp_pred, mlp_conf, trans_pred, trans_conf, attn_weights):
        ...
        consensus_conf = np.clip(consensus_conf, 0.0, 1.0)
...
class ModelStorage:
    def __init__(self, pipeline, memory_name, db_path='activity_log.db'):
        ...
        self.setup_hnsw_table()
        self.setup_head_memory_table()
    ...
    def setup_hnsw_table(self):
        try:
            ...
            c.execute('''CREATE TABLE IF NOT EXISTS hnsw_weight_storage
                     (id INTEGER PRIMARY KEY AUTOINCREMENT,
                      memory_name TEXT,
                      model_type TEXT,
                      weights TEXT,
    ...
    def setup_head_memory_table(self):
        try:
            try:
                db_path = self.get_database_path()            
                conn = sqlite3.connect(db_path)
            ...
            c.execute('''CREATE TABLE IF NOT EXISTS head_memory_storage
                     (id INTEGER PRIMARY KEY AUTOINCREMENT,
                      memory_name TEXT,
                      model_type TEXT,
                      weights TEXT,
    ...
    def save_hnsw_memory(self, memory_name, model_type='HNSW'):
        try:
            db_path = self.get_database_path()
            conn = sqlite3.connect(db_path)
        except:
        ...
        memories = self.pipeline.model2.memory.hnsw_arch.to_dict()    
        memories_json = json.dumps(memories, default=str)
        try:
            c.execute("""
                INSERT INTO hnsw_weight_storage 
                (memory_name, model_type, weights, is_active)
                VALUES (?, ?, ?, ?)
            """, (memory_name, model_type, memories_json, 1))
        
            c.execute("""
                UPDATE hnsw_weight_storage 
                SET is_active = 0 
                WHERE memory_name = ? AND id != last_insert_rowid()
            """, (memory_name,)) 
            ...
            print('[||] All HNSW Setup dictionary saved!')

        except Exception as e:
            print(f'[-] Cant save Weights due to: {e}')
            pass 

    def save_memory_head(self, memory_name, model_type='memory_head'):
        try:
            db_path = self.get_database_path()
            conn = sqlite3.connect(db_path)
        except:
        ...
        memories = self.pipeline.model2.memory.to_dict()    
        memories_json = json.dumps(memories, default=str)
        try:
            c.execute("""
                INSERT INTO head_memory_storage 
                (memory_name, model_type, weights, is_active)
                VALUES (?, ?, ?, ?)
            """, (memory_name, model_type, memories_json, 1))
        
            c.execute("""
                UPDATE head_memory_storage 
                SET is_active = 0 
                WHERE memory_name = ? AND id != last_insert_rowid()
            """, (memory_name,)) 
            ...
            print('[||] All kNN-Memory heads dictionary saved!')

        except Exception as e:
            print(f'[-] Cant save Memory heads due to: {e}')
            pass    

    def load_hnsw_setup(self, memory_name):
        ...
        c.execute("""
        SELECT weights FROM hnsw_weight_storage 
        WHERE memory_name = ? AND is_active = 1
        """, (memory_name,))               
        ...
        if result:
            print('[+] HNSW Setup memory is found!')     
        else:
        ...
        try:
            ...
            self.pipeline.model2.memory.hnsw_arch.dim = weights.get('dim', 0)
            self.pipeline.model2.memory.hnsw_arch.M = weights.get('M', 0)
            self.pipeline.model2.memory.hnsw_arch.ef_construction = weights.get('ef_construction')
            self.pipeline.model2.memory.hnsw_arch.metric = weights.get('metric', [])
            self.pipeline.model2.memory.hnsw_arch.seed = weights.get('seed', 0)
            self.pipeline.model2.memory.hnsw_arch.vectors = weights.get('vectors', [])
            self.pipeline.model2.memory.hnsw_arch.neighbors = weights.get('neighbors', [])
            self.pipeline.model2.memory.hnsw_arch.max_level_of = weights.get('max_level_of', None)
            self.pipeline.model2.memory.hnsw_arch.entry_point = weights.get('entry_point', None)
            self.pipeline.model2.memory.hnsw_arch.max_level = weights.get('max_level', 0)
            self.pipeline.model2.memory.hnsw_arch._rng = weights.get('rng_state', 0)

            print('[+] All HNSW Setups are loaded!')

        except Exception as e:
            print(f'[!] Cant load any Weights due to: {e}')
            traceback.print_exc()     

    def load_head_memory_setup(self, memory_name):
        """Load weights from database. Returns True if found."""
        try:
            db_path = self.get_database_path()            
            conn = sqlite3.connect(db_path)
        ...
        c.execute("""
        SELECT weights FROM head_memory_storage 
        WHERE memory_name = ? AND is_active = 1
        """, (memory_name,))               
        ...
        if result:
            print('[+] Memory head setup is found!')       
        else:
            print(f'[=] No saved weights for {memory_name}')
            return False
        ...
        try:
            ...
            self.pipeline.model2.memory.n_heads = weights.get('n_heads', 0)
            self.pipeline.model2.memory.head_dim = weights.get('head_dim',0)
            self.pipeline.model2.memory.capacity = weights.get('capacity', 0)
            self.pipeline.model2.memory.k = weights.get('k', 0)
            self.pipeline.model2.memory.metric = weights.get('metric', [])
            self.pipeline.model2.memory.evict_batch = weights.get('evict_batch', [])
            self.pipeline.model2.memory.seed = weights.get('seed', None)
            self.pipeline.model2.memory._head_seeds = weights.get('head_seeds', [])
            self.pipeline.model2.memory.indices = weights.get('indices', [])
            self.pipeline.model2.memory.values = weights.get('values')
            self.pipeline.model2.memory._count = weights.get('count')

            print('[+] All Per-Head Memory Setups are loaded!')
...
class QueryNode:
    ...
    def __init__(self, pipeline, memory_name, storage,
        ...
        # hardened RateLimiter class fixed earlier this session
    ...
    def _evaluate_node_agreement(self, node):
        ...
        # confidence_threshold: normalized into [0,1] via sigmoid instead
...
class IntegratedPipeline:
    ...
    def _get_ensemble_confidence_for_true_class(self, X, Y, input_ids=None, eps=1e-3):
        ...
        input_ids : sequence_inputs — same format self.model2 (Transformer)
                    consumes with embedded=True. Optional; MLP-only
        ...
        if hasattr(self, 'mlp') and self.mlp is not None:
            try:
                ...
                print(f'[⚠️] MLP forward failed in ensemble confidence: {e} '
                    f'— MLP contribution will be skipped')
                mlp_probs = None

        # ── Transformer confidence, batched ──────────────────────
        trans_probs = None
            ...
            try:
                ...
                if len(input_ids_arr) != n_samples:
                    print(f'[⚠️] input_ids length ({len(input_ids_arr)}) != '
                        ...
                        f'— Transformer contribution will be skipped')
                else:
                ...
                print(f'[⚠️] Transformer forward failed in ensemble confidence: '
                    f'{e} — Transformer contribution will be skipped')
                trans_probs = None

        if mlp_probs is None and trans_probs is None:
            print('[⚠️] Neither MLP nor Transformer produced usable probs — '
    ...
    def transformer_utilities(self, X_val=None, y_val=None, X_provided= None, X_raw=None, y_true=None, rules=None, 
        ...
        if not unsuitable_training:
            ...
            if not unsuitable:
                self.model3.train(X, y, X_val=X_val, y_val=y_val, epochs=self.mlp_training_epochs, lr=self.mlp_lr, max_samples_for_focused_fit=max_samples_for_focused_fit)
            else:
...
# ----------------------------------------------------------------------
# Multi-epoch tracking for SNN
# ----------------------------------------------------------------------
 
class DiagnosticsTracker:
    """
    Call `.record(net, epoch, loss=..., test_acc=...)` once per epoch (right
    after a forward/backward pass, before zero_grad) to accumulate a
    per-epoch, per-layer history. `.summary()` prints a compact trajectory
    table; `.history` gives raw arrays for custom plotting.
    """
    ...
    def __init__(self):
        ...
        self.loss = []
        self.test_acc = []
        self.per_layer = {}   # layer_index -> dict of lists
 
    def record(self, net, epoch, loss=None, test_acc=None):
        self.epochs.append(epoch)
        self.loss.append(loss)
        self.test_acc.append(test_acc)
 
        report = network_report(net)
        for entry in report:
            idx = entry["index"]
            bucket = self.per_layer.setdefault(idx, {
                "type": entry["type"], "mean_rate": [], "threshold": [],
                "weight_norm": [], "grad_norm": [], "n_dead": [], "n_saturated": [],
            })
            bucket["mean_rate"].append(entry.get("mean_rate"))
            bucket["threshold"].append(entry.get("threshold"))
            bucket["weight_norm"].append(entry["norm"])
            bucket["grad_norm"].append(entry.get("grad_W_norm"))
            bucket["n_dead"].append(entry.get("n_dead"))
            bucket["n_saturated"].append(entry.get("n_saturated"))
 
    def summary(self, last_n=5):
        """Print loss/accuracy plus the last `last_n` epochs of per-layer
        firing rate & threshold, so you can see the trend at a glance."""
        print(f"[=] Epochs recorded: {len(self.epochs)}")
        if self.loss and self.loss[-1] is not None:
            print(f"[=] loss: {self.loss[0]:.4f} -> {self.loss[-1]:.4f}")
        if self.test_acc and self.test_acc[-1] is not None:
            print(f"[=] test_acc: {self.test_acc[0]*100:.1f}% -> {self.test_acc[-1]*100:.1f}%")
        for idx, bucket in self.per_layer.items():
            if bucket["type"] != "LIFDenseLayer":
                continue
            rates = [r for r in bucket["mean_rate"][-last_n:] if r is not None]
            thresh = [t for t in bucket["threshold"][-last_n:] if t is not None]
            print(f"[=] layer {idx} last {len(rates)} epochs -- "
                  f"rate: {['%.3f' % r for r in rates]}  "
                  f"threshold: {['%.3f' % t for t in thresh]}")
...
class SNNUtility:
    ...
    def seed_everything(self, seed):
        """Return a seeded numpy Generator; also seeds the legacy global state
        for any code that still calls np.random.* directly."""
        np.random.seed(seed)
    ...
    def clip_grad_norm(self, layers, max_norm, eps=1e-6):
        """Global-norm gradient clipping across every parameter of every layer,
        analogous to torch.nn.utils.clip_grad_norm_. Scales all gradients by
        the same factor so relative gradient direction is preserved."""
        total_sq = 0.0
        grad_refs = []
        for layer in layers:
            _, grads = layer.parameters()
...
# SNN Network Build
#-----------------
def build_snn_network(n_in, n_hidden, n_classes, rng, decay=0.95, 
                      threshold=0.1, input_noise_std=0.0, reduction='mean'):
    layer1 = LIFDenseLayer(n_in, n_hidden, decay=decay, threshold=threshold, rng=rng)
    drop1 = SpikeDropout(p=0.2, rng=rng)
    readout = ReadoutLayer(n_hidden, n_classes, decay=decay, rng=rng)
    net = SNNNetwork([layer1, drop1, readout], readout_reduction=reduction,
                      input_noise_std=input_noise_std, rng=rng)
    return net, layer1
...
def stdp_pretrain(layer, encoder, X_unlabeled, rng, n_steps=200, batch_size=32,
                   a_plus=0.01, a_minus=0.011, tau_pre=15.0, tau_post=15.0):
    """
    Unsupervised pretraining loop for a single LIFDenseLayer using pair-based
    STDP. Only spikes are used -- no labels, no loss, no optimizer step.
    """
    # STDP is defined for weights that stay in a bounded, non-negative range;
    # re-init the layer's weights accordingly before pretraining with it.
    layer.W[:] = rng.uniform(0.0, 0.4, size=layer.W.shape)
    stdp = STDPLearner(layer.in_features, layer.out_features,
    ...
    for step in range(n_steps):
        xb = X_unlabeled[rng.integers(0, n, size=batch_size)]
        pre_spikes = encoder.encode(xb)
        post_spikes = layer.forward(pre_spikes)   # plain LIF forward pass; no backward needed
        stdp.apply(layer.W, pre_spikes, post_spikes)
 
    return layer
 
def layer_firing_stats(layer, dead_thresh=1e-4, saturated_thresh=0.99):
    """
    Per-neuron firing-rate breakdown for one LIFDenseLayer, using the spike
    history recorded during its most recent forward() call.
 
    Returns a dict with mean/min/max/std rate, per-neuron rate array, and
    counts of "dead" (never fires) and "saturated" (fires almost every
    step) neurons -- the two failure modes discussed for LIF layers.
    """
    if not isinstance(layer, LIFDenseLayer):
        raise TypeError("layer_firing_stats expects a LIFDenseLayer")
    if layer._cache is None:
        raise RuntimeError("layer has no forward-pass cache yet; call forward() first")
 
    spikes = layer._cache["spike_hist"]              # (T, B, F_out)
    per_neuron_rate = spikes.mean(axis=(0, 1))        # (F_out,)
 
    return {
        "mean_rate": float(per_neuron_rate.mean()),
        "min_rate": float(per_neuron_rate.min()),
        "max_rate": float(per_neuron_rate.max()),
        "std_rate": float(per_neuron_rate.std()),
        "per_neuron_rate": per_neuron_rate,
        "n_neurons": per_neuron_rate.shape[0],
        "n_dead": int(np.sum(per_neuron_rate < dead_thresh)),
        "n_saturated": int(np.sum(per_neuron_rate > saturated_thresh)),
        "threshold": float(layer.threshold),
    }
 
 
def weight_stats(layer):
    """Basic weight-matrix health: mean, std, L2 norm, and max abs value."""
    W = layer.W
    return {
        "mean": float(W.mean()),
        "std": float(W.std()),
...
def grad_norm_stats(layer):
    """L2 norm of the most recently computed weight gradient (call after
    backward(), before the optimizer's zero_grad() clears it)."""
    if getattr(layer, "grad_W", None) is None:
        return {"grad_W_norm": None}
    out = {"grad_W_norm": float(np.linalg.norm(layer.grad_W))}
    if getattr(layer, "grad_b", None) is not None:
        out["grad_b_norm"] = float(np.linalg.norm(layer.grad_b))
    return out
 
 
def network_report(net, dead_thresh=1e-4, saturated_thresh=0.99):
    """
    One-shot health snapshot across every layer in an SNNNetwork, right
    after a forward()/backward() call. Returns a list of per-layer dicts
    (LIFDenseLayer entries include firing stats; ReadoutLayer entries only
    include weight/grad stats, since it never spikes).
    """
    report = []
    for i, layer in enumerate(net.layers):
        entry = {"index": i, "type": type(layer).__name__}
        if isinstance(layer, LIFDenseLayer):
            entry.update(layer_firing_stats(layer, dead_thresh, saturated_thresh))
            entry.update(weight_stats(layer))
            entry.update(grad_norm_stats(layer))
...
def print_report(report):
    """Pretty-print the output of network_report()."""
    for entry in report:
        if "mean_rate" in entry:
            print(f"[{entry['index']}] {entry['type']:14s} "
                  f"rate(mean/min/max/std)={entry['mean_rate']:.3f}/"
                  f"{entry['min_rate']:.3f}/{entry['max_rate']:.3f}/{entry['std_rate']:.3f}  "
                  f"dead={entry['n_dead']}/{entry['n_neurons']}  "
                  f"saturated={entry['n_saturated']}/{entry['n_neurons']}  "
                  f"thresh={entry['threshold']:.3f}  "
                  f"|W|={entry['norm']:.3f}  |gW|={entry['grad_W_norm']}")
        else:
            print(f"[{entry['index']}] {entry['type']:14s} "
                  f"|W|={entry['norm']:.3f}  |gW|={entry['grad_W_norm']}")
 
 
def check_health(report, target_rate=0.15, rate_tolerance=0.5, max_weight_norm=None):
    """
    Turns a network_report() into a list of plain-English warnings, e.g.
    dead/saturated neurons, firing rate far from a target, or weight/grad
    norms that look like they're exploding. Returns [] if nothing stands out.
    """
    warnings = []
    for entry in report:
        tag = f"layer {entry['index']} ({entry['type']})"
        if "n_dead" in entry and entry["n_dead"] > 0:
            warnings.append(f"{tag}: {entry['n_dead']}/{entry['n_neurons']} neurons are dead (never fire)")
        if "n_saturated" in entry and entry["n_saturated"] > 0:
            warnings.append(f"{tag}: {entry['n_saturated']}/{entry['n_neurons']} neurons are saturated (fire almost every step)")
        if "mean_rate" in entry:
            lo, hi = target_rate * (1 - rate_tolerance), target_rate * (1 + rate_tolerance)
            if not (lo <= entry["mean_rate"] <= hi):
                warnings.append(f"{tag}: mean firing rate {entry['mean_rate']:.3f} is outside "
                                 f"[{lo:.3f}, {hi:.3f}] around target {target_rate}")
        if entry.get("grad_W_norm") and not np.isfinite(entry["grad_W_norm"]):
            warnings.append(f"{tag}: grad_W contains NaN/Inf")
        if max_weight_norm is not None and entry["norm"] > max_weight_norm:
            warnings.append(f"{tag}: weight norm {entry['norm']:.2f} exceeds {max_weight_norm}")
    return warnings
 
def dynamic_test_size(y, min_val_per_class=5, min_frac=0.1, max_frac=0.3):
    """
...
def _prepare_train_val_split(X, y, min_val_per_class=5,
    ...
    dynamic_test_bed = dynamic_test_size(
    ...
    try:
        ...
        X_train, X_val, y_train, y_val = train_test_split(
            X_train, y_train, test_size=dynamic_test_bed + min_frac, random_state=42
        )
    except ValueError as e:
...
# ----------------------------------------------------------------------
# 1. Basic softmax confidence
# ----------------------------------------------------------------------
 
def softmax_probs(logits, temperature=1.0):
    """logits: (B, C) -> probs: (B, C), rows sum to 1."""
...
def predict_with_confidence(logits, temperature=1.0, class_names=None):
    """
    logits: (B, C) raw network output.
    Returns a list of per-sample dicts: predicted class (index and/or
    name), its probability, the full probability vector, and the
    distribution's entropy (in nats) as an uncertainty measure -- entropy
    is 0 for a fully confident one-hot prediction and ln(C) for a
    uniform/maximally-uncertain one.
    """
    ...
    for i in range(B):
        ...
        results.append({
            "pred_index": pred_idx,
            "pred_name": class_names.get(pred_idx, "Unknown") if class_names is not None else pred_idx,
            "confidence": float(p[pred_idx]),
            "probs": p,
            "entropy": entropy,
            "entropy_frac": entropy / max_ent,   # 0 (certain) .. 1 (uniform/uncertain)
        })
    return results
...
class TemperatureScaler:
    """
    Fits a single scalar temperature T > 0 that divides the logits before
    softmax: probs = softmax(logits / T). This is Guo et al. (2017)'s
    temperature scaling -- the simplest, most robust post-hoc calibration
    method, which cannot change *accuracy* (T doesn't change argmax) but
    substantially improves how trustworthy the reported confidence is.
 
    T > 1 softens (reduces) over-confident probabilities (most common case
    for networks trained to near-zero training loss, like a memorizing/
    overfit SNN). T < 1 sharpens under-confident probabilities.
 
    Fit on a held-out VALIDATION set (never the training set -- a network
    is closest to perfectly "calibrated" on data it memorized, which would
    give a useless T ~ 1 even for a badly overconfident model on new data).
    """
 
    def __init__(self):
        self.temperature = 1.0
 
    def fit(self, val_logits, val_labels, lr=0.01, n_steps=200):
        """
        Simple 1-D gradient descent on negative log-likelihood w.r.t. T,
        implemented directly (no autograd needed for a single scalar).
        """
        T = 1.0
        ...
        for _ in range(n_steps):
            probs = softmax_probs(val_logits, temperature=T)
            # d(NLL)/dT for softmax(logits/T): see Guo et al. derivation;
            # computed here via finite difference for simplicity/robustness.
            eps = 1e-3
            nll_plus = self._nll(val_logits, labels, T + eps)
            nll_minus = self._nll(val_logits, labels, max(T - eps, 1e-3))
            grad = (nll_plus - nll_minus) / (2 * eps)
            T = max(T - lr * grad, 1e-3)
    ...
    def _nll(logits, labels, T):
        probs = softmax_probs(logits, temperature=T)
        B = logits.shape[0]
        return -np.mean(np.log(probs[np.arange(B), labels] + 1e-12))
 
    def apply(self, logits):
        return softmax_probs(logits, temperature=self.temperature)
...
def expected_calibration_error(probs, labels, n_bins=10):
    """
    ECE: bins predictions by confidence, and measures the average gap
    between confidence and actual accuracy within each bin. 0 = perfectly
    calibrated (e.g. among predictions made with 80% confidence, 80% are
    actually correct); higher = worse. This is the standard metric from
...
def reliability_diagram_data(probs, labels, n_bins=10):
    """Returns (bin_centers, bin_accuracy, bin_confidence, bin_counts) for
    plotting a reliability diagram (accuracy vs. confidence, per bin)."""
    labels = np.asarray(labels)
    confidences = probs.max(axis=1)
    preds = probs.argmax(axis=1)
    correct = (preds == labels).astype(np.float64)
...
# ----------------------------------------------------------------------
# 4. SNN-specific: Monte-Carlo confidence via encoding/dropout stochasticity
# ----------------------------------------------------------------------
 
def mc_confidence(net, encoder, x, n_samples=20, temperature=1.0,
                   enable_dropout=True, class_names=None):
    """
    Unlike a standard feed-forward net, an SNN fed by a stochastic encoder
    (e.g. PoissonEncoder) produces a *different* spike train every time
    you re-encode the same input. Re-running the forward pass several
    times and averaging gives you:
      - a smoother, more reliable mean confidence than any single
        stochastic pass (the single-pass confidence is noisier than it
        looks, since it depends on which random spike train was drawn)
      - a genuine uncertainty estimate: the spread (std) of predicted
        probabilities across passes tells you how sensitive the
        prediction is to encoding noise -- a proxy for how "on the edge"
        this input is, similar in spirit to Monte-Carlo Dropout
        (Gal & Ghahramani, 2016).
 
    x : (B, F) static input (same array fed to `encoder.encode()`).
    Returns a list of per-sample dicts with mean confidence, std across
    samples, mean entropy, and the full per-pass probability array.
    """
    was_training = net.training
    if enable_dropout:
        net.train()   # keep SpikeDropout active for extra MC variability
    else:
        net.eval()
    ...
    for _ in range(n_samples):
        ...
        logits = net.forward(spikes)
        all_probs.append(softmax_probs(logits, temperature))
    all_probs = np.stack(all_probs, axis=0)   # (n_samples, B, C)
    ...
    for i in range(B):
        ...
        results.append({
            ...
            "pred_name": class_names.get(pred_idx, 'Unknown') if class_names is not None else pred_idx,
            "confidence": float(p[pred_idx]),
            "confidence_std": float(std_probs[i, pred_idx]),
            "probs": p,
            "probs_std": std_probs[i],
            "entropy": entropy,
            "entropy_frac": entropy / max_ent,
            "n_samples": n_samples,
        })
    return results
...
def print_confidence_report(results, top_k=3, max_print_threshold=50):
    """readable printout for a list of dicts from
    predict_with_confidence() or mc_confidence()."""
    pred_cache = []
    for i, r in enumerate(results):
        order = np.argsort(-r["probs"])[:top_k]
        top_str = ", ".join(f"{cls}:{r['probs'][cls]*100:.1f}%" for cls in order)
        std_str = f"  (±{r['confidence_std']*100:.1f}%)" if "confidence_std" in r else ""
        pred_cache.append(r['pred_name'])
        if i <= max_print_threshold:
            print(f"[>] Sample {i:3d} -> predicted={r['pred_name']}  "
                f"|| Confidence={r['confidence']*100:.1f}%{std_str}  "
                f"|| Entropy={r['entropy_frac']*100:.0f}%  "
                f"|| Top-{top_k}: [{top_str}]")
    return pred_cache



def train_snn_network(X=None, y=None, net=None, hidden_layer=None, 
         epochs=50, batch_size=32, n_steps=20, 
         n_hidden=64, lr=2e-3, seed=0, pretrain_steps=200, unsupervised_stdp=False,
         random_state=42):
    tracker = DiagnosticsTracker()
    utility = SNNUtility()
    cache = {}

    rng = utility.seed_everything(seed)
    X_train, X_test, y_train, y_test, _, _ = _prepare_train_val_split(X=X, y=y, random_state=random_state)
    n_in = X_train.shape[1]
    n_classes = int(y_train.max() + 1)
    if net is None and hidden_layer is None:
        net, hidden_layer = build_snn_network(n_in, n_hidden, n_classes, rng)

    encoder = PoissonEncoder(n_steps=n_steps, max_rate=1.0, rng=rng)
    optimizer = Adam(lr=lr, weight_decay=1e-5)
    homeostasis = ThresholdHomeostasis(target_rate=0.15, adjust_rate=0.05)

    # ---- Phase 1: unsupervised STDP pretraining (labels NOT used) ----
    if unsupervised_stdp:
        if n_in == n_hidden:
            utility.stdp_pretrain(hidden_layer, encoder, X_train, rng, n_steps=pretrain_steps,
                        batch_size=batch_size)
            print(f"[=] After STDP pretraining: hidden layer mean firing rate = "
                f"{hidden_layer._cache['spike_hist'].mean():.3f}")
            per_neuron_rate = hidden_layer._cache["spike_hist"].mean(axis=(0, 1))  # shape (n_hidden,)
            print("[=] Dead neurons:", (per_neuron_rate < 1e-4).sum(), "/", len(per_neuron_rate))
            per_neuron_rate = hidden_layer._cache["spike_hist"].mean(axis=(0, 1))
            print("[=] Neuron firing rates - min:", per_neuron_rate.min(), "max:", per_neuron_rate.max(), "std:", per_neuron_rate.std())
        else:
            print('[>] Unsupervised STDP pretraining skipped due to mismatched Input_shapes[1] size with the amount of network hidden layers')

    # ---- Phase 2: supervised BPTT fine-tuning of the whole network ----
    for epoch in range(1, epochs + 1):
        net.train()
        epoch_losses = []
        for xb, yb in iterate_minibatches(X_train, y_train, batch_size, rng):
            spikes = encoder.encode(xb)
            loss, logits = net.train_step(
                spikes, yb, optimizer,
        ...
        homeostasis.step(net.layers)

        # Evaluation
        net.eval()
        test_spikes = encoder.encode(X_test)
        train_spikes = encoder.encode(X_train)
        preds, _ = net.predict(test_spikes)
        train_preds, _ = net.predict(train_spikes)
        test_acc = utility.accuracy(preds, y_test)
        train_acc = utility.accuracy(train_preds, y_train)
        print(f"[=] Epoch {epoch:2d} || train_loss={np.mean(epoch_losses):.4f}  "
              f"train_acc={train_acc*100:5.1f} || test_acc={test_acc*100:5.1f}% || hidden_threshold={hidden_layer.threshold:.3f}")
              
        tracker.record(net, epoch, loss=loss, test_acc=test_acc)

    report = network_report(net)
    print_report(report)
    warnings = check_health(report, target_rate=0.15)
    if warnings:
        print("\n".join(warnings))

    return encoder, net, report, warnings


def snn_predict(X=None, y=None, label_map=None, encoder=None, net=None, random_state=42, max_print_threshold=50):
    utility = SNNUtility()
    reverse_map = None

    if X is not None and y is not None:
        print(f"[🔍] Processing X and y samples for Spiking Neural Network...")
        if isinstance(X, (str, np.str_)):
            clean_str = str(X).replace('[', '').replace(']', '')
            X = np.fromstring(clean_str, sep=' ')
        if isinstance(X, np.ndarray) and np.issubdtype(X.dtype, np.character):
        ...
        raise AttributeError('[>] Missing Samples during SNN prediction! All X and Y samples must be required for SNN prediction!')
    ...
    try:
        test_spikes = encoder.encode(X)
        preds, logits = net.predict(test_spikes)
        if reverse_map is not None:
            # SNN-specific: uncertainty from the encoder's own stochasticity
            mc_results = mc_confidence(net, encoder, X, n_samples=20, class_names=reverse_map)
            print(f'[>] Displaying {max_print_threshold} predictions over {len(X)} samples...')
            _ = print_confidence_report(mc_results, max_print_threshold=max_print_threshold) 
        else:
            print('[>] Label map is Missing during SNN prediction, returning SNN Logits as is without any label mapping...')
    except Exception as e:
        print(f'[⚠️] Error occured during SNN prediction: {e}')
        traceback.print_exc()
        return None, 0.0
...
class PipelinePredictionManager:
    ...
    def knn_forward_inference(self, X, y, label_map=None, memory_metric='euclidean', training=False, batch_size=2, train_mode='fixed_backward', lr=0.1):
        ...
        if self.pipeline.model2 is not None:
            self.pipeline.knn_tf = KNNAugmentedTransformer(
                                 ...
                                 memory_capacity=self.pipeline.memory_capacity, memory_k=self.pipeline.memory_k, 
                                 memory_metric=memory_metric
            )
        else:
            self.pipeline.model2 = Transformer(
                    vocab_size=self.pipeline.vocab_size,
                    d_model=self.pipeline.transformer_d_model,
                    n_heads=self.pipeline.transformer_heads,
                    num_classes=num_classes
            )
            self.pipeline.knn_tf = KNNAugmentedTransformer(
                                 vocab_size=self.pipeline.vocab_size, d_model=self.pipeline.transformer_d_model, n_heads=self.pipeline.transformer_heads, num_classes=num_classes,
        ...
        try:
            if X is None or y is None:
                raise AttributeError('[!] Some Sample is None or missing!')

            if X is not None and y is not None:
                print(f"[🔍] Processing X and y samples for kNN Augmented Transformer...")
                if isinstance(X, (str, np.str_)):
                    clean_str = str(X).replace('[', '').replace(']', '')
                    X = np.fromstring(clean_str, sep=' ')
                if isinstance(X, np.ndarray) and np.issubdtype(X.dtype, np.character):
            ...
            if label_map is None:
                ...
                trans_pred_indices = np.mean(trans_probs, axis=0)
                trans_pred_idx = np.argmax(trans_pred_indices)
                predict_confidence = trans_pred_indices[trans_pred_idx]
    ...
    def _compute_need_ensemble_method(self, anisotropy, AME, error_counts,
        ...
        if error_counts is not None and len(error_counts) > 0:
            ...
            max_error = error_counts[worst_class_idx]
    ...
    def _get_voted_results(self, probs, reverse_map, n_samples):
        pred_cache = []
        _cache = {}
        cache = []
        for i in range(n_samples):
            try:
                each_idx = probs[i].argmax()
            except:
                each_idx = np.argmax(probs[:len(reverse_map)-1])
            try:
                confidence = float(probs[each_idx])   
            except:
                confidence = float(probs[i][len(reverse_map)-1]) if isinstance(probs[i], (float, int)) else 0.0
            
            pred_cache.append(each_idx)
            _cache[i] = (each_idx, confidence)
            
        voted_chosen_label = max(set(pred_cache), key=pred_cache.count)
        conf_cache = []
        for i in range(len(_cache)):
            if _cache[i][0] == voted_chosen_label:
                confidence = _cache[i][1] 
                conf_cache.append(confidence)

        float_counts = Counter(conf_cache)
        voted_confidence, _ = float_counts.most_common(1)[0]      

        return voted_chosen_label, voted_confidence
    ...
    def advanced_prediction_method(self, titles=None, label_map=None, rules=None,
        ...
        try:
            ...
            if dataset is not None:
                input_ids, _ = self.pipeline.input_encoding(dataset)
            elif dataset is not None and X is not None:
                input_ids, _ = self.pipeline.input_encoding(dataset)
                ...
                # Uses actual whole X not X_train to avoid cheat learning.
                input_ids = self.pipeline._features_to_sequence(X)
            ...
            for i in range(len(results)):
                each_label = results[i].get('predicted') if results else 'Unknown'
                each_confidence = results[i].get('confidence') if results else 0.0
                each_agreement = results[i].get('models_agree') if results else False

                prediction_cache.append(each_label)
                confidence_cache[i] = (each_label, each_confidence)
                agreement_cache[i] = (each_label, each_agreement)

            voted_chosen_label = max(set(prediction_cache), key=prediction_cache.count)
            conf_cache = []
            for i in range(len(confidence_cache)):
                if confidence_cache[i][0] == voted_chosen_label:
                    confidence = confidence_cache[i][1] 
                    conf_cache.append(confidence)
            ...
            voted_agreement = []
            for i in range(len(agreement_cache)):
                if agreement_cache[i][0] == voted_chosen_label:
                    get_agreement = agreement_cache[i][1] 
                    voted_agreement.append(get_agreement)

            counts = Counter(voted_agreement)
            agreement_counts = counts[True]
            models_agreement = agreement_counts > counts[False]

            if not isinstance(voted_chosen_label, str):
                ...
                voted_chosen_label = results[0].get('predicted') if results else None
                voted_confidence = results[0].get('confidence') if results else 0.0

            if isinstance(voted_chosen_label, int) or isinstance(voted_chosen_label, np.integer):
                voted_chosen_label = str(voted_chosen_label)
                
            if isinstance(voted_confidence, (np.ndarray, list)):
                voted_confidence = np.mean(voted_confidence)

            if not need_ensemble_method:
                print('[=] Getting Results...')
                if not results[0].get('models_agree', True) and self.pipeline.use_transformer:
                    trans_conf_cache = []
                    for i in range(len(results)):
                        trans_confidence = results[i].get('trans_confidence', 1e-8)
                        trans_conf_cache.append(trans_confidence)
                    
                    trans_counts = Counter(trans_conf_cache)
                    trans_confidence, _ = trans_counts.most_common(1)[0]
                    confidence = (voted_confidence + trans_confidence / 2) + 1e-5
                    print(f'[=] Calibrating confidence around frequent Transformer confidence distributions: {confidence:.1%}')
                ...
                if isinstance(voted_chosen_label, str) and voted_chosen_label.startswith("unknown") or float(voted_confidence) < self.pipeline.confidence_threshold:
                    if voted_chosen_label is None or voted_chosen_label.startswith('unknown'):
                        voted_chosen_label = 'Unknown'
                        voted_confidence = 1.0 - voted_confidence  #Invert confidence for unknown class
                        print(f"\n[⚠️] Frequent predicted label is {voted_chosen_label} with uncertain confidence: {voted_confidence:.1%}. Consider more consistent data for the model to learn from.")
                    else:
                        print(f"\n[🎯] Frequent Predicted label: {voted_chosen_label} || With Certain Confidence: {voted_confidence:.1%}")

                return results, voted_chosen_label, voted_confidence

            else:
                if models_agreement and voted_confidence > self.pipeline.confidence_threshold and not voted_chosen_label.startswith("Unknown"):
                    print(f'[>] Agreement count: {agreement_counts}, Both models agree frequently on predicted label: {voted_chosen_label}')
                    print(f"\n[🎯] Proper Confidence of Final chosen label for input: {voted_chosen_label} || Confidence: {voted_confidence:.1%}")
                    return results, voted_chosen_label, voted_confidence
                    
                # Only recalibrate if models disagreed
                elif results and not models_agreement or not self.pipeline.agreement and self.pipeline.use_transformer:

                    need_peer_condition = not models_agreement and self.pipeline.peer_assistance_threshold > 0.3 and self.pipeline.use_transformer
                    print("\n[⚠️] Disagreement detected between MLP and Transformer predictions. Usig calibrated probabilities for final decision.")
                    if voted_confidence < self.pipeline.confidence_threshold and need_peer_condition:
                        ...
                        final_idx, voted_confidence = self._get_voted_results(final_probs, reverse_map, len(X))
                        ...
                        voted_chosen_label = reverse_map.get(final_idx, f"unknown_{final_idx}")
                    ...
                    elif not models_agreement and voted_confidence > self.pipeline.confidence_threshold:
                        ...
                        if self.pipeline.use_transformer and need_ensemble_method:
                            ...
                            target_cache = []
                            for i in range(len(target_pred_indices)):
                                target_cache.append(target_pred_indices[i])
                                
                            counter = Counter(target_cache)
                            predicted_idx, _ = counter.most_common(1)[0]

                            final_probs = self.calibration_penalized_check(target_probs, predicted_idx)
                            final_idx, voted_confidence = self._get_voted_results(target_probs, reverse_map, len(X)) 
                            original_idx = final_idx 

                            if final_idx > len(reverse_map):
                                final_idx = int(np.argmax(target_probs[:len(reverse_map)-1]))
                                print(f"[⚠️] Clamping {final_idx} → {final_idx}")

                            final_idx = int(final_idx)          
                            voted_chosen_label = reverse_map.get(final_idx, f"unknown_{final_idx}")

                elif voted_confidence < self.pipeline.confidence_threshold and not self.pipeline.agreement and not results[0].get('models_agree', True):
                    if trans_probs is not None:
                    ...
                    voted_confidence < 0.4 or  # Low confidence
                    not models_agreement or  # Disagreement
                    attn_quality < 0.4
                    )
                    ...
                    if use_simple_prediction:
                        ...
                        final_idx, confidence =self._get_voted_results(final_probs, reverse_map, len(X)) 
                        original_idx = final_idx
            ...
            if voted_confidence > 0.8:
                voted_confidence = (voted_confidence + performance_score) / 2 
                print(f'[⚡] Confidence calibrated to be around: {confidence}')

            if isinstance(voted_chosen_label, str) and voted_chosen_label.startswith("unknown") or float(voted_confidence) < self.pipeline.confidence_threshold:
                if voted_chosen_label.startswith("unknown"):
                    voted_chosen_label = 'Unknown'
                    voted_confidence = 1.0 - voted_confidence  #Invert confidence for unknown class

                print(f"\n[⚠️] Final prediction is {voted_chosen_label} with uncertain confidence: {voted_confidence:.1%}. Consider more consistent data for the model to learn from.")
            else:
                print(f"\n[🎯] Final chosen label for input: {voted_chosen_label} || Confidence: {voted_confidence:.1%}")  
            ...
            try:
                ...
                sec_final_idx, sec_confidence = self._get_voted_results(consecutive_probs, reverse_map, len(X))
                ...
                if 'sec_predicted' in results and results['sec_predicted'] is None and sec_chosen_label:
                    ...
                    if confidence > results[0]['confidence']:
                        results[0]['predicted'] = voted_chosen_label
                        results[0]['confidence'] = voted_confidence
                ...
                results[0]['sec_predicted'] = voted_chosen_label
                results[0]['sec_confidence'] = voted_confidence
            ...
            try:
                ...
                voted_chosen_label = results[0]['predicted']
                voted_confidence = results[0]['confidence']
            except Exception as error:
                print(f'[= ! =] Error in all prediction method: {error}')
                traceback.print_exc()
                results, voted_chosen_label, voted_confidence = None, None, 0.0
        ...
        if sec_chosen_label and sec_correct > correct:
            ...
            voted_chosen_label = sec_chosen_label
        else:
            print(f'[⚡] Final Prediction: {voted_chosen_label} with confidence: {voted_confidence:.2%}')
            voted_chosen_label = voted_chosen_label
