...
def print_confidence_report(results, top_k=3):
    """Human-readable printout for a list of dicts from
...
def train_snn_network(X=None, y=None, epochs=50, batch_size=32, n_steps=20, 
...
def snn_predict(X=None, y=None, label_map=None, encoder=None, net=None, random_state=42):
    utility = SNNUtility()
    reverse_map = None
    ...
    try:
        ...
        if reverse_map is not None:
            # 3. SNN-specific: uncertainty from the encoder's own stochasticity
            mc_results = mc_confidence(net, encoder, X, n_samples=20, class_names=reverse_map)
            print_confidence_report(mc_results) 
        else:
