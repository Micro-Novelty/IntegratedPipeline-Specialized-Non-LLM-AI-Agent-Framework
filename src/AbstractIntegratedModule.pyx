    ...
    def advanced_batch_prediction(self, test_titles, label_map, rules, X=None, y=None, api_key=None, client_ip=None):
        try:
            ...
            if not self.pipeline.intents:
                ...
                result, chosen_label, confidence = self.advanced_prediction_method(self.prediction_manager
        ...
        for idx, result in enumerate(results):
            ...
            if result.get('expected'):
                ...
                if 'sec_predicted' in result:
                    ...
                    if result['sec_predicted'] == result['expected']:
                        self.pipeline.accurate_cache_lookup.add_verified(
                            X_samples, input_ids, 
                            result['sec_predicted'], result['sec_confidence'], result['index'],
                ...
                if result['predicted'] == result['expected']:
                    self.pipeline.accurate_cache_lookup.add_verified(
                        X_samples, input_ids, 
                        result['predicted'], result['confidence'], result['sec_index'],
                        source='automatic_verified')      
